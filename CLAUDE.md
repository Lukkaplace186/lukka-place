...
- **Viewport fetching (Rightmove/Zillow model) — `/listings` only.** The map no longer plots the list page's 12 cards. `components/ListingsMap.js` asks `GET /api/listings/map` (`web/lib/listings.js` `getMapMarkers`) for every approved listing matching the URL's filters inside the padded viewport, re-asking on `idle` (350ms debounce, in-flight request aborted, no request at all while the view stays inside the last padded box). Lightweight markers only; the preview card fetches the full listing through `/api/listings?ids=` when a pin is tapped.
  - **Location filters give way to the viewport.** With a box, `commune`/`quartier`/`radius` are dropped and every other filter still applies — so "2 chambres à Bandal" opens on Bandal's real geocoded viewport (one client `Geocoder` call per place, cached) and panning out loads more 2-bedroom listings beyond it. A search naming no place opens on `?extent=1`, the box around everything matching. A non-location filter change keeps the view.
  - **The list pane is unchanged** — still paginated on the full filters, location included.
  - **Positions**: stored coordinates, else the commune centroid (flagged `approximate`), else no pin and counted `unlocated` — never a made-up point. `/listings` no longer geocodes listings in the browser at all; `PropertyMap`'s client cascade above now serves only the detail page. The map badge states listings in view, how many sit on a centroid, and how many match but cannot be placed.
...
- **Markers**: classic `google.maps.Marker` instances (not the newer `AdvancedMarkerElement` — that needs a Cloud Console Map ID and wasn't necessary here; `Marker` still works, just prints a deprecation warning), each rendered as a dense price tag. **Clustering is back on `/listings`** (`@googlemaps/markerclusterer`, `SuperClusterAlgorithm`, max zoom 15): a royal-blue count bubble whose number is listings, so a building pin counts its units. Above zoom 15 every pin is a price tag again, and co-located pins are still separated by the fan described above. The detail page's single-listing map does not cluster.
