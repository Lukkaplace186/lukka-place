 ...
 * activation token.
 *
 * `agents.id` is GENERATED BY DEFAULT AS IDENTITY (checked against the live
 * schema), so the INSERT omits it and takes the sequence's next value —
 * never MAX(id) + 1 (see the comment at the INSERT and
...
async function upsertAgentFromWhatsApp({ waId, fullName, agencyName }) {
  ...
  try {
    ...
    if (existing.length) {
      ...
      // The id comes from agents' identity sequence. It used to be
      // MAX(id) + 1, which never advanced the sequence (web signups then
      // collided on the primary key) and, once admins can delete agents,
      // would hand a deleted agent's id — and every engine row still
      // carrying it — to the next sender.
      const { rows: idRows } = await client.query(
        `INSERT INTO agents
           (username, phone, status, agency_name, phone_verified_at, onboarding_source,
            activation_token_hash, activation_expires_at, created_at, updated_at)
         VALUES ($1, $2, 1, $3, NOW(), 'whatsapp', $4, $5, NOW(), NOW())
         RETURNING id`,
        [fullName || digits, digits, agencyName || null, hashToken(token), expiresAt],
      );
      agentId = idRows[0].id;
      created = true;
    }
