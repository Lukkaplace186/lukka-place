...
- **Public pages live in the `app/(site)/` route group**; `app/admin` and `app/api` sit outside it. The shell (`Header` / `SideRail` / `Footer` / `BottomNav`) is in `app/(site)/layout.js`, and the root layout is deliberately bare. Before this, everything nested in one root layout and `/admin` rendered the public header, footer and bottom nav *underneath* its own chrome. Route groups don't change URLs.
- **Spacing contract, defined once in `app/(site)/layout.js`**: `pt-16` clears the fixed `h-16` Header, `lg:pl-[76px]` clears the SideRail, `pb-16 lg:pb-0` clears the BottomNav. Don't re-implement these per page — four pages used to compensate ad hoc with different values.
- **Breakpoint handoff is exact**: `SideRail` is `hidden lg:flex`, `BottomNav` is `lg:hidden`. Never both, never neither — `BottomNav` was `sm:hidden` with nothing replacing it, leaving 640–1024px with no primary nav at all. Both read the same `components/navItems.js`.
...
- `Hero.js` uses `-mt-16` to bleed under the fixed header so the photo reaches the top of the viewport and the header can sit transparently over it.
...
- **Per-listing agent contact is not viable yet, and it isn't a data gap — it's a schema limitation.** `properties.agent_id` (FK, NULL on every row) points at a real `agents`/`agent_infos` structure in the Laravel schema (phone, image, name, visibility flags) — that's the right mechanism for multi-agency contact info, not new flat columns on `properties`. But `agents` currently holds exactly one row, and it's test/placeholder data (`testings@gmail.com` / username `testing`), and `agents.phone` is a 32-bit `integer` column that cannot hold this system's real E.164 phone format (`wa_id`, e.g. `243997123456` — overflows a 32-bit int). Writing a truncated number into it would produce a *wrong* `tel:`/`wa.me` link, not just an empty one. Real agent accounts (with real email/password) must be created by the team through Laravel, not synthesized here — creating login-capable accounts is outside what this assistant does. Until the `agents.phone` type is resolved and real accounts exist, every listing card correctly falls back to the single central `WhatsAppCTA` number, exactly as today.
