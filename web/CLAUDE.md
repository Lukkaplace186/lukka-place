...
- **`advance_months` / `commission_months` do not exist on `properties`** — verified directly against the live schema, which carries `deposit_months` and nothing else. The engine already parses and stores all three separately in SQLite (Kinshasa's "Garantie : 3 + 1 + 1" is a deposit, an advance and a commission, and summing them overstates the deposit — see the root CLAUDE.md note), but `syncListingToPostgres` can only write the one column that exists. `lib/listingView.js`'s `entryTerms()` and the KeyFacts cell that renders it are already written against all three, so the full "3 + 1 + 1 mois" breakdown needs an `ALTER TABLE`, the engine sync change, a backfill from SQLite, and the two columns added to `SELECT_FIELDS` — no component change. Until then that cell renders the deposit alone, and **nothing fills the gap with a "standard" advance or commission**: there is no standard, and a guessed figure here is a number a customer would budget against. Note too that legacy rows may already carry a SUMMED value in `deposit_months` (7 approved listings show 4, 5 or 6 months) from before the engine split the fields; those are indistinguishable from a genuine long deposit without re-reading the original WhatsApp text.
