...
- **`features` (`text[]`) exists on `properties` and is rendered** — the "Caractéristiques principales" list on the detail page (`components/listings/PropertyDescription.js`, `lib/descriptionParser.js`). Added by the engine's `scripts/migrate-listing-features.js`. Populated on every listing published since the engine's extraction started emitting it, plus whatever that script's backfill could honestly recover from the agent's original WhatsApp message. **It is NULL on most existing rows and that is the designed state, not a gap to paper over**: the backfill deliberately refuses to invent bullets out of a public description, because 0 of the 48 approved descriptions are list-shaped (checked directly) and sentence-splitting one produces bullets that restate the paragraph under them. A NULL row falls back to the AMENITY_KEYWORDS pass, which carries its own weaker caption.
