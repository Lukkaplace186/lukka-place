...
export default async function AdminSubscriptionsPage({ searchParams }) {
  const t = await getT();
  const featuredQuery = String((await searchParams)?.fq || '').slice(0, 80);
  const [memberships, featuredPricings, featuredIds, approvedListings, vendors, packages, planRequests] =
    await Promise.all([
      getMemberships(),
      getFeaturedPricings(),
      getFeaturedPropertyIds(),
      // Server-side search over every approved public listing — the old fixed
      // list of the 50 newest made listing #51 impossible to feature.
      searchFeaturableListings({ q: featuredQuery, limit: 25 }),
      getVendors(),
      getPackages(),
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mb-3 text-xs text-ink-45">{t('admin.subscriptions.featuredSearchHint')}</p>
        <form method="get" className="mb-3 flex max-w-md items-center gap-2">
          <input
            type="search"
            name="fq"
            defaultValue={featuredQuery}
            placeholder={t('admin.subscriptions.featuredSearchPlaceholder')}
            className="u-focus-ring h-9 flex-1 rounded-md border border-line bg-white px-2.5 text-sm text-ink"
          />
          <button type="submit" className="h-9 rounded-md border border-line bg-white px-3 text-sm font-medium text-ink hover:bg-canvas-alt">
                  ...
                  return (
                    <tr key={listing.id} className="border-b border-line last:border-b-0">
                      <td className="px-4 py-2.5 text-ink">
                        ...
                        <div className="text-xs text-ink-45">#{listing.id}{listing.reference ? ` · Réf. ${listing.reference}` : ''}</div>
                      </td>
                      <td className="px-4 py-2.5">
                        {isFeatured ? (
