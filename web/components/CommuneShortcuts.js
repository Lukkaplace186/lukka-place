 ...
 *
 * Each pill carries its real verified-listing count ("Gombe · 42"), which
 * is the whole point of the row: eight identical name-only pills are eight
 * identical guesses, while the count turns each one into a real shortcut
 * and doubles as a trust signal. The number is `getPopularCommunes()`'s own
 * COUNT(*) behind the approved filter — never a decorative figure — and it
 * gets `.u-tabular` so a row of pills doesn't go ragged on digit widths.
 *
      ...
      <div className="mx-auto max-w-[1240px] px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <div className="no-scrollbar -mx-4 flex items-center gap-2.5 overflow-x-auto px-4 sm:mx-0 sm:flex-wrap sm:px-0">
          ...
          {communes.map(({ commune, count }) => (
            ...
            >
              {commune}
              <span className="u-tabular text-ink-45" aria-label={`${count} annonces vérifiées`}>
                · {count}
              </span>
            </Link>
          ))}
