 ...
 */
export function buildListingSocialCopy(listing, { typeText = null, url = null } = {}) {
  const lines = [];

  const purpose = listing.purpose === 'sale' ? 'À VENDRE' : listing.purpose === 'rent' ? 'À LOUER' : null;
  const head = [purpose, typeText].filter(Boolean).join(' · ');
  if (head) lines.push(`🏠 ${head}`);

  const place = [listing.quartier, listing.commune].filter(Boolean).join(', ');
  if (place) lines.push(`📍 ${place}`);

  lines.push(`💰 ${formatPrice(listing.price, listing.purpose, listing.price_period)}`);
